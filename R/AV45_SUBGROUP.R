...
nsfa6.clust = Mclust(df_scores$NSFA_6, G=2)
df_scores$NSFA_6_CLASS = as.factor(nsfa6.clust$classification - 1)
...
# target.slope = 'CSF_ABETA_slope'
# target = 'CSF_TAU_closest_AV45_1'
# target.slope = 'CSF_TAU_slope'
# target = 'CSF_PTAU_closest_AV45_1'
# target.slope = 'CSF_PTAU_slope'
# target = 'UCB_FS_HC.ICV_AV45_1'
# target.slope = 'UCB_FS_HC.ICV_slope'
# target = 'CORTICAL_SUMMARY_prior'
# target.slope = 'CORTICAL_SUMMARY_change'

# filter by diag
# diag_filter = c('N','SMC')
# df_scores = df_scores[which(df_scores$Diag.AV45 %in% diag_filter),]
...
target_suvr_form = as.formula(paste(target,'~ CORTICAL_SUMMARY_prior'))
target_nsfa6_form = as.formula(paste(target,'~ NSFA_6')) 
targetslope_suvr_form = as.formula(paste(target.slope,'~ CORTICAL_SUMMARY_prior'))
targetslope_nsfa6_form = as.formula(paste(target.slope,'~ NSFA_6')) 

# diag violin plots
p1 = ggplot(suvr.pos, aes_string('Diag.AV45','NSFA_6')) +
  ...
  ylab('NSFA_6 Score') +
  xlab('Diagnosis')
p2 = ggplot(suvr.pos, aes_string('Diag.AV45','CORTICAL_SUMMARY_prior')) +
  geom_violin(trim=TRUE,aes(fill=Diag.AV45),show.legend=FALSE) +
  coord_flip() + 
  theme(axis.title.x=element_text(size=20),
        axis.title.y=element_text(size=20),
